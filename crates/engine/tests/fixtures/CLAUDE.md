...
| ----------------------- | ------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| `mock-normal.py`        | Echoes args and returns `-32700` for malformed JSON without exiting. `--protocol-version <v>` overrides the `initialize` reply's version | happy-path, raw-fuzzer rejection, and `protocol_version` negotiation tests |
...
| `mock-crash.py`         | Exits on 1% of calls by default; `--crash-after N` deterministically exits on call N                                | error rate accuracy and terminal-path regressions                                                                                              |
...
| `mock-stateless-http.py` | Experimental implementation fixture for the final MCP 2026-07-28 tools/discovery/request-header subset over Streamable HTTP: rejects `initialize`, returns `supportedVersions` from `server/discover`, and requires version/method/name headers plus per-request `_meta`; `--lazy-deadlock` hangs every `tools/call` | `stateless` (ADRs 0019 and 0023) |
